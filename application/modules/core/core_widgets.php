...
class Core_Widgets extends MY_Controller
{
    ...
    public function recent_news($widget = []) {
        ...
        $this->db->select('CONCAT_WS("", content.cat_url, content.url) as full_url, content.id, content.title, prev_text, full_text, publish_date, showed, comments_count, author, category.name as cat_name, content.cat_url', FALSE);
        $this->db->join('category', 'category.id=content.category');
        $this->db->where('post_status', 'publish');
        $this->db->where('prev_text !=', 'null');
        $this->db->where('full_text !=', 'null');
